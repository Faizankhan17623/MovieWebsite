    ...
    exports.Createuser = async(req,res)=>{
        try {
            ...
            const {name,password,confirmpass,email,usertype="Viewer",number,otp} = req.body
...
exports.updateUserName = async(req,res)=>{
    try {
        ...
        if(!Finding){
            return res.status(404).json({
                message:"The username is not preseent or you are not loged in",
                success:false
            })
        }
        ...
        if(userName === Finding.userName){
            return res.status(400).json({
                message:"The old and the new username is the same",
                success:false,
                data:userName
            })
        }else{
            let updating = await USER.findByIdAndUpdate(req.USER.id,{userName:userName})
            await mailSender(email,"The username is been updated",updateUsername(Finding.username,updating.username))
            return res.status(200).json({
                message:"The username is been updated",
                success:true,
                data:updating.username
            })
...
exports.updatePassword= async(req,res)=>{
    try {
        ...
        const decrypting =  bcrypt.compare(newPassword,password,10)
        if(decrypting === false){
            return res.status(400).json({
                message:"The old and the new passwords are the same",
                success:false
            })
        }

        const hashing = await bcrypt.hash(newPassword,10)
        const updating = await USER.findByIdAndUpdate(req.USER.id,{password:hashing,confirmpass:hashing},{new:true})
...
exports.updateNUmber= async(req,res)=>{
    try {
        ...
        const UPdatingNumber = await USER.findByIdAndUpdate(req.USER.id,{number:newNumber},{new:true})
        console.log("THis is the new updated number",UPdatingNumber)
        await mailSender(email,"The phone number is been updated",updateNumber(Finding.userName,newNumber))
        return res.status(200).json({
